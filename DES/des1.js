...
//обратная перестановка IP-1 таблица
...
function stringToBinary(str, length = 8) {
    return str.split('').map(c => c.charCodeAt().toString(2).padStart(8, '0'));
}

function binaryToString(binary, length = 8) {
    let result = '';
    for (let i = 0; i < 8; i++) {
...
function permutateTable(bit = 8) {
    const init = [...Array(bit).keys()].map(x => {
        return [...Array(bit).keys()].map(i => {
            return (bit * x) + i + 1;
        });
    });
    let y = [...Array(bit).keys()].map(i => {
        let h = [];
        init.forEach(x => h.push(x[i]));
        return h;
    });
    let odd = y.filter(
        (x, xi) => xi % 2 === 0).map((x) => x.reverse()
        );
    let even = y.filter(
        (x, xi) => xi % 2 === 1).map((x) => x.reverse()
        );
    return [...even, ...odd];
}
...
function calcKeyPermutation2(c0, d0) {
    return keyExtended(c0, d0).map((x, xi) => {
        const c = x[`c${xi + 1}`];
        const d = x[`d${xi + 1}`];
        return permutation(`${c}${d}`, keyPermutationTable2);
    })
}
...
function ciphering(l, r, K) {
    ...
    K.forEach((k, ki) => {
        ...
        const B = calcSBox(A);
        const PB = permutation(B, permutationTable);
...
function desEncrypt(plaintext, key) {
    ...
    //Генерация ключа
    const K = hexToBinary(key);
    const Kplus = permutation(K, keyPermutationTable1);

    //разбиение на 2 блока С0 и D0 по 28
    const C0 = Kplus.substring(0, 28);
    const D0 = Kplus.substring(28, 56);
    const Keys = calcKeyPermutation2(C0, D0);

    //Шифрование
    const encipher = ciphering(L0, R0, Keys);

    //Обратная перестановка
    const cipherText = permutation(encipher.join(''), invertPermutateTable);
...
function desDecrypt(cipherText, key) {
    ...
    const C0 = Kplus.substring(0, 28);
    const D0 = Kplus.substring(28, 56);
    const Keys = calcKeyPermutation2(C0, D0).reverse();

    //Дешифрование
    const decipher = ciphering(L0, R0, Keys);

    //Обратная перестановка
    const plainTextBinary = permutation(decipher.join(''), invertPermutateTable);
